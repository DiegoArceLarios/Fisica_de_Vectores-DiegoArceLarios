...
import {Text, View, StyleSheet, ScrollView, Alert, SafeAreaView, Platform, TouchableOpacity, ImageBackground, Image} from 'react-native'
...
export default class Escalar extends Component{
    ...
    igual(){
        ...
        if(operacion===1){
            ...
            hj= number2-number5  
            hi= number3-number6
            m1=((number1**2)+(number2**2)+(number3**2))**(1/2)
            m2=((number4**2)+(number5**2)+(number6**2))**(1/2)
            m3=((hk**2)+(hj**2)+(hi**2))**(1/2)
            mCos=((m3**2)-((m1**2)+(m2**2)))/(-2*(m2*m1))
            res=m1*m2*mCos
                ...
                <ScrollView style={{flex: 1, borderBottomWidth: 3, borderColor: 'white'}}>
                    <View style={{justifyContent: 'center', marginTop: 40, margin: 10}}>
                        ...
                        <Text style={{color: 'white', fontSize: 15, fontWeight: 'bold'}}>
                            o = multiplicación vector-vector escalar
                        </Text>
                        <View style={{flexDirection: 'row', marginTop: 40, backgroundColor: '#676767'}}>
                        <TouchableOpacity style={[styles.numeros,{backgroundColor: '#F193EA'}]} onPress={()=>this.decimal()}>
                            <Text>
                                ÷10
                            </Text>
